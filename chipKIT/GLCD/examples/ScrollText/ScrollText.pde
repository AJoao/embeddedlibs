...
void loop()
{
  ...
  GLCD.setCursor(63-((strlen(phrases[line])*6)/2),8);

  // Print the line
  GLCD.print(phrases[line]);
  ...
  for(int i=0; i<12; i++)
  {
    ...
    GLCD.update();
    // It's not readable if we go at full speed.  A 150ms delay per pixel line is good enough.
